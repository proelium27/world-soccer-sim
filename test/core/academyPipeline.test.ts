...
import { releaseExpiredContracts } from "../../src/core/freeAgency.js";
import { processLoanReturns, type ActiveLoan } from "../../src/core/loans.js";
import {
...
/** The ranking the offseason takes before it changes anything. */
const rank = (t: StoredTeam, players: Player[]) => academyRanking(t, players, SEASON);
...
describe("academyDuePids", () => {
  it("leaves out a prospect signed from free agents, whose ordinary deal simply lapses", () => {
    const graduate = kid(1, ACADEMY_GRADUATION_AGE - 1, 70);
    const prospect = kid(2, ACADEMY_GRADUATION_AGE + 1, 90);
    const players = [graduate, prospect];
    const t = team(players);
    expect(academyDuePids(t, players, SEASON)).toEqual(new Set([graduate.pid]));

    // So he isn't promoted at the professional cut, however good he is...
    const out = resolveAcademyCheckpoints([t], players, TID, SEASON, NEXT, rank(t, players));
    expect(out.promoted).toEqual([graduate.pid]);
    // ...he leaves through the ordinary expiry at step 1.5 instead...
    const lapsed = releaseExpiredContracts([t], players, SEASON, academyDuePids(t, players, SEASON));
    expect(lapsed[0].academyRoster).toEqual([graduate.pid]);
    // ...and the preview doesn't claim him, which is what lets the page warn his deal is up.
    expect(projectAcademyCheckpoints(t, players, [], SEASON).has(prospect.pid)).toBe(false);
  });
});
...
describe("resolveAcademyCheckpoints", () => {
  ...
  it("releases every graduate when the senior roster is full", () => {
    const t = team(players, ROSTER_CAP);
    const out = resolveAcademyCheckpoints([t], players, TID, SEASON, NEXT, rank(t, players));
  ...
  it("returns the league unchanged when nobody is due", () => {
    ...
    const pool = [young];
    const out = resolveAcademyCheckpoints(teams, pool, TID, SEASON, NEXT, rank(teams[0], pool));
  ...
  it("does nothing for a club nobody manages", () => {
    ...
    const out = resolveAcademyCheckpoints(teams, players, -3, SEASON, NEXT, new Map());
    expect(out.teams).toBe(teams);
  });

  it("ranks on the order taken before the rollover, not on potentials the rollover re-estimates", () => {
    const a = kid(1, ACADEMY_GRADUATION_AGE - 1, 90);
    const b = kid(2, ACADEMY_GRADUATION_AGE - 1, 35);
    const before = [a, b];
    const t = team(before, ROSTER_CAP - 1);
    expect(projectAcademyCheckpoints(t, before, [], SEASON).get(a.pid)!.outcome).toBe("promote");

    // Progression re-estimates every potential before the cut runs; swap them.
    const after = [{ ...a, potential: 35 }, { ...b, potential: 90 }];
    const out = resolveAcademyCheckpoints([t], after, TID, SEASON, NEXT, rank(t, before));
    expect(out.promoted).toEqual([a.pid]);
  });
});
...
describe("trimAcademyToCap", () => {
  ...
  it("stays over the cap rather than cutting a kid who isn't at a decision", () => {
    ...
    const teams = [team(players)];
    const out = trimAcademyToCap(teams, players, TID, NEXT, rank(teams[0], players));
...
describe("projectAcademyCheckpoints", () => {
  it("previews the promotions the rollover makes", () => {
    ...
    const resolved = resolveAcademyCheckpoints([t], players, TID, SEASON, NEXT, rank(t, players));
  ...
  it("counts senior room the way the rollover's loan returns and contract expiries leave it", () => {
    const OTHER = 7;
    const g1 = kid(1, ACADEMY_GRADUATION_AGE - 1, 90);
    const g2 = kid(2, ACADEMY_GRADUATION_AGE - 1, 60);
    const g3 = kid(3, ACADEMY_GRADUATION_AGE - 1, 35);
    const borrowed = kid(20, 22, 60, SEASON + 2); // in on a loan that ends: goes back
    const lapsing = kid(21, 29, 60, SEASON); // own deal up: goes
    const homeA = kid(22, 21, 60, SEASON + 2); // out on a loan that ends: comes home
    const homeB = kid(23, 21, 60, SEASON + 2);
    const away = kid(24, 21, 60, SEASON + 3); // out on a loan that runs on: stays away
    const players = [g1, g2, g3, borrowed, lapsing, homeA, homeB, away];
    const loan = (pid: number, parentTid: number, loaneeTid: number, returnSeason: number): ActiveLoan => ({
      pid, parentTid, loaneeTid, startSeason: SEASON, seasons: returnSeason - SEASON, returnSeason, fee: 0,
    });
    const loans = [
      loan(borrowed.pid, OTHER, TID, NEXT),
      loan(homeA.pid, TID, OTHER, NEXT),
      loan(homeB.pid, TID, OTHER, NEXT),
      loan(away.pid, TID, OTHER, NEXT + 1),
    ];
    const base = team([g1, g2, g3], ROSTER_CAP - 3);
    // One place free today, and still exactly one at the cut: two players leave
    // and two come home. Counting the roster as it stands would say two.
    const t: StoredTeam = { ...base, roster: [...base.roster, borrowed.pid, lapsing.pid] };

    // The rollover itself: loan returns (step 1), loans outliving their deal end
    // (1.25), expiry skipping loans and checkpoint kids (1.5), then the cut (5.0).
    const back = processLoanReturns([t], loans, [], NEXT);
    const contract = new Map(players.map((p) => [p.pid, p.contract.expiresSeason]));
    const runningOn = back.activeLoans.filter((l) => contract.get(l.pid)! > SEASON).map((l) => l.pid);
    const atCut = releaseExpiredContracts(
      back.teams, players, SEASON, new Set([...runningOn, ...academyDuePids(back.teams[0], players, SEASON)]),
    );
    const resolved = resolveAcademyCheckpoints(atCut, players, TID, SEASON, NEXT, rank(t, players));
    expect(resolved.promoted).toEqual([g1.pid]);

    const preview = projectAcademyCheckpoints(t, players, loans, SEASON);
    for (const pid of resolved.promoted) expect(preview.get(pid)!.outcome).toBe("promote");
    for (const pid of resolved.released) expect(preview.get(pid)!.outcome).toBe("release");
  });
  ...
  it("frees a lapsing prospect's academy place before counting the intake against the cap", () => {
    const low = kid(1, ACADEMY_SCHOLARSHIP_AGE - 1, 30);
    const prospect = kid(2, ACADEMY_GRADUATION_AGE + 1, 60);
    const others = Array.from(
      { length: ACADEMY_ROSTER_CAP - USER_ACADEMY_INTAKE_MAX - 1 },
      (_, i) => kid(100 + i, USER_ACADEMY_ENTRY_AGE, 60, NEXT + 1),
    );
    const players = [low, prospect, ...others];
    const t = team(players);
    // One over after a full intake, were the prospect still there. He isn't.
    expect(projectAcademyCheckpoints(t, players, [], SEASON).get(low.pid)!.outcome).toBe("keep");

    // And the rollover agrees: the lapse, a full intake, then the trim.
    const lapsed = releaseExpiredContracts([t], players, SEASON, academyDuePids(t, players, SEASON));
