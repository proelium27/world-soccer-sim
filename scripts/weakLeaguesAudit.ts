...
// (Argentina 7.5 and the US 10.5, both a compensated head start for their
// 30-club split top flights — see COUNTRY_STRENGTH_OFFSET). Where one shares an
// offset with a European neighbour (Mexico with Belgium) the pair is 0 points
// apart, and any rung under RESOLVABLE_STEP, half-point ones included, reads as
// converged — gated on the cross-seed mean only, like the one-point rungs.
// KEEP THIS IN OFFSET ORDER: a rung compares each league with the one above it.
const WEAK_LADDER = [
  "France", "Brazil", "Argentina", "Netherlands", "Portugal", "United States",
  "Belgium", "Mexico", "Turkey", "Greece", "Scotland", "Serbia",
];
