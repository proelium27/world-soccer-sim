...
export function loanGateFor(
  ...
  return (player, team, seasons) => {
    ...
    // is his, so it says so rather than blaming his club.
    const appeal = appealScore(player, userCtx, { stature: parentCtx.stature, club: parentCtx }, { loan: true });
    if (appeal.refused) return "He won't drop to your level";
    const reservation = keepValueToClub(player, parentCtx);
    if (valueToClub(player, userCtx) * Math.max(0, 1 + appeal.score) < reservation * (1 + LOAN_MIN_SURPLUS)) {
