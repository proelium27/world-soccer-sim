...
export function progressPlayer(
  ...
  const stepped = stepRatings(rng, player.ratings, age, player.pos, minutesFactor, player.pid, player.heightCm);
  ...
  const steppedPotential = estimatePotential(
    rng, stepped, steppedOvr, age, steppedPos, player.heightCm, player.pid,
  );
