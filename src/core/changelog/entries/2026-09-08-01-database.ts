...
const entry: ChangelogEntry = {
  ...
  items: [
    ...
    "One thing worth knowing about the potential column: it's your scouts' estimate, the same one you see everywhere else, and both the sorting and the filtering use the top of that estimate rather than the real number. Otherwise sorting the table would quietly tell you something your scouts haven't worked out yet. The top end because it's the number you can actually see — sort by potential and the right-hand figure counts down the column, so the order makes sense. Market value is worked out from the middle of the band instead, since a price should be what he's worth on balance, not at his best.",
    "**That fix reaches further back than the new page.** Youth Intake, Academy, Free Agents and Loans were all sorting their potential column by the real number behind the estimate. Youth Intake was the bad one: it opened sorted by true potential with five contracts to give out, so the top five rows were always exactly the five to sign, and the estimate on screen was decoration. Free Agents used the real number to decide who made the list at all. All four now rank on the estimate, which is what they were showing you the whole time.",
