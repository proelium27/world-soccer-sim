      ...
      <div style={{ maxWidth: "56rem" }}>
        ...
        <Section id="overview" title="Overview">
          ...
          <p>
            ...
            two ready-made ones. Both give every club in the game its real name and colours, and
            most of them their real badge. "Real clubs and players" also puts real squads in most top flights, and "Real
            clubs" keeps the made-up players. Pick one and load it with Import. They're separate
            downloads rather than part of the game, which is why you fetch them instead of
        ...
        <Section id="pages" title="The Pages">
          ...
          <ul>
            ...
            <li><strong>Standings</strong>. The league table, plus each club's current OVR/POT. A season dropdown lets you pull up any past season&apos;s final table next to the current one. The champion&apos;s row is highlighted, and the <a href="#cup">Continental Cup</a> and <a href="#shield">Continental Shield</a> qualification places are shaded. The promotion and relegation places are drawn as lines across the table: a solid line where the table settles it and a dashed one where a playoff still has to, with a key underneath naming the division you go up or down to. Before a ball has been kicked the position column is blank and nothing is shaded, since with every club on zero points the order isn&apos;t anything yet.</li>
          ...
          <p>
            ...
            Watchlist and both national-team squad pages carry the same switch above their
            tables. <strong>Overview</strong> is the page as it always was, with prices, wages
            and contracts. <strong>Attributes</strong> puts all fourteen ratings beside the name,
            so you can line ten strikers up on finishing instead of on overall.{" "}
            <strong>Performance</strong> shows what a player actually did in the league: games,
            minutes, average match rating, goals, assists, xG, tackles, interceptions and saves,
            with a dropdown for the last few seasons. Click any heading to sort by it.
          </p>
          <p>
            Two things about sorting on those views. Sorting by match rating puts the regulars
            first: anyone who played under half as many games as the busiest player on the list
            sorts below them, because a 9.0 from one cameo off the bench isn&apos;t a season.
            And on the Player Pool the sort runs over every eligible player in your country
            before the list is cut to the top forty, so sorting by rating really does surface
            the in-form player nobody rates rather than reshuffling the forty best by overall.
            Which view you&apos;re on is part of the page&apos;s address, so a sorted attribute
            sheet is a link you can bookmark or come back to.
          </p>
        </Section>
        ...
        <Section id="youth" title="The Youth Academy">
          ...
          <p>
            ...
            so what reaches it is genuinely what nobody wanted. Anyone 21 or younger there can go
            straight into your academy on the academy stipend rather than a senior wage, as long as
            the academy has one of its 20 places free and he's willing to join you. Your own academy
        ...
        <Section id="godmode" title="God Mode">
          ...
          <ul>
            ...
            <li><strong>Change how the continental competitions are played.</strong> The God Mode page has a Cup Formats tab with a card for each club competition your world plays: the Continental Cup, the Continental Shield and the Americas Cup, each set on its own. Pick a style (<em>Groups and a Round of 16</em> is the old Champions League, <em>Long league phase</em> is eight games each and a Round of 16 like today's, <em>Straight knockout</em> skips the group games, <em>One-off ties</em> drops the second legs), or open <em>Fine-tune</em> and set it yourself: how it opens (one league phase, groups of four, or straight into the knockout), 4, 6 or 8 league-phase games, where the knockout starts (semi-finals, quarter-finals or a Round of 16), whether there's a playoff round, and one or two legs, whether away goals break a level tie, and how many clubs are in it. The final is always one match. With away goals on, a two-legged tie that finishes level on aggregate goes to whoever scored more away from home rather than to extra time. If it's still level after extra time, goals scored in extra time by the side playing away count too, so a late one there can send them through. The size setting changes who qualifies, which is the one setting here that does: ask for a bigger competition and every league sends more of its table, ask for a smaller one and it's the best clubs only. The stronger leagues keep sending more than the weaker ones either way, and the competition below shifts down the tables to make room, so making the Cup bigger has the Shield taking clubs from further down. If the leagues can't fill the size you asked for, it plays at the biggest size they can, and the page tells you. Under the styles, a line says in plain words what the competition will look like next season. Groups send the top two through, plus the best third-placed sides when the numbers need topping up, and a group's two qualifiers can't meet each other straight away. A straight knockout gives the top seeds a bye when the field is bigger than the bracket. A change starts with next season's draw, so anything already under way finishes the way it started. Apart from the size, the same clubs qualify whatever you pick, and prize money is paid for the stages each format actually plays: a Round of 16 win is worth half a quarter-final win, and a straight knockout pays the entry fee on the first tie. The fixtures stay off the domestic cup matchdays. Like the award formulas, the setting stays in force if you switch God Mode off, and the competition's page says so.</li>
